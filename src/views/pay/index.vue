<template>
  <div class="pay">
    ...
    <div class="address">
      ...
      <div class="right-icon" @click="$router.push('/addresslist')">
        <van-icon name="arrow" />
      </div>
    </div>
    ...
    <div class="pay-list">
      ...
      <!-- 买家留言 -->
      <div class="buytips">
        <textarea v-model="remark" placeholder="选填：买家留言（50字内）" id="" cols="30" rows="10"></textarea>
      </div>
    </div>
...
export default {
  ...
  computed: {
    // 地址拼接
    longAddress () {
      if (!this.addressList || !this.addressList.region) {
        return ''
      }
      const region = this.addressList.region
      return region.city + region.province + region.region
    },
    mode () {
  ...
  methods: {
    ...
    async getAddressList () {
      ...
      this.addressList = list.find((item) =>
        item.address_id === this.defaultId
      )
    },
    // 由购物车或商品详情页结算而来，获取结算的商品数据
    async getOrderList () {
      if (this.mode === 'cart') {
